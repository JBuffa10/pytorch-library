...
class ResNet101(nn.Module):
  def __init__(self, pretrained: bool = True, num_features_out: int = 256):
    ...
    self.resent101 = torchvision.models.resnet101(pretrained=self.pretrained)

  def get_features(self):
    features = list(self.resent101.children())

    input = nn.Sequential(*features[:3])
    conv2 = nn.Sequential(*([features[3]] + list(features[4].children())))
    conv3 = features[5]
    conv4 = features[6]
    conv5 = features[7]
